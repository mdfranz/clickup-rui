...
async fn run_workload_loop<A: ClickUpApi + Clone + 'static>(
    ...
    'reload: loop {
        ...
        let mut excluded_statuses: HashSet<String> =
            ["completed", "closed", "backlog", "done", "cancelled"]
                .iter()
            ...
            let filtered_tasks: Vec<Task> = member_idx
                .map(|idx| {
                    members[idx]
                        ...
                        .iter()
                        .filter(|task| {
                            !excluded_statuses.contains(&task.status.status.to_lowercase())
                        })
                        .cloned()
                        .collect()
                })
                ...
                match (&detailed_task, &comments) {
                    (Some(det), Some(coms)) => {
                        ...
                        detail_lines.push(Line::from(vec![
                            ...
                            Span::styled(
                                det.status.status.to_uppercase(),
                                Style::default()
                                    .add_modifier(Modifier::BOLD)
                                    .fg(status_color),
                            ),
                        ...
                        detail_lines.push(Line::from(vec![
                            ...
                            Span::styled(
                                format_task_date(&det.date_updated),
                                Style::default().fg(crate::ui::styles::COLOR_FG),
                            ),
                        ]));

                        let task_url =
                            format!("https://app.clickup.com/t/{}/{}", cfg.workspace_id, det.id);
                        ...
                        let wrapped_desc =
                            crate::util::format::wrap_text_by_words(desc_text, detail_pane_width);
                        ...
                        if coms.is_empty() {
                            ...
                            for c in coms {
                                ...
                                let wrapped_comment = crate::util::format::wrap_text_by_words(
                                    &c.comment_text,
                                    detail_pane_width.saturating_sub(2),
                                );
                                for line in wrapped_comment {
                                    ...
                                    for seg in segments {
                                        match seg {
                                            ...
                                            crate::util::format::TextSegment::Link {
                                                url,
                                                text: _,
                                            } => {
                                                spans.push(Span::styled(
                        ...
                        max_right_scroll =
                            detail_lines.len().saturating_sub(detail_pane_height) as u16;
                ...
                let member_items: Vec<ListItem> = visible_member_indices
                    ...
                    .map(|&idx| {
                        ...
                        let shown = m
                            ...
                            .iter()
                            .filter(|t| {
                                !excluded_statuses.contains(&t.status.status.to_lowercase())
                            })
                            .count();
                        ...
                        if show_comment_editor {
                                ...
                                && key.modifiers.contains(KeyModifiers::CONTROL)
                            {
                                if !comment_buffer.trim().is_empty() && !filtered_tasks.is_empty() {
                                    ...
                                    if api
                                        .create_task_comment(&task_id, &comment_buffer)
                                        .await
                                        .is_ok()
                                    {
                                    ...
                                    if idx < list_statuses.len() && !filtered_tasks.is_empty() {
                                            ...
                                            .is_ok()
                                        {
                                            ...
                                            if let Some(m) = members.iter_mut().find(|m| {
                                                ...
                                                if let Some(t) =
                                                    m.tasks.iter_mut().find(|t| t.id == task_id)
                                                {
                        ...
                        match key.code {
                            ...
                            KeyCode::Char('f')
                                if active_pane == WorkloadPane::Tasks
                                    || active_pane == WorkloadPane::Members =>
                            {
