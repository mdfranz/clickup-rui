...
use ratatui::widgets::{
    Block, Borders, Clear, List as RatatuiList, ListItem, ListState, Padding, Paragraph, Wrap,
};
...
pub async fn run_browse<A: ClickUpApi + Clone + 'static>(
    api: &A,
    all_flag: bool,
    mine_only: bool,
) -> Result<()> {
...
async fn run_browse_loop<A: ClickUpApi + Clone + 'static>(
    ...
    'reload: loop {
        ...
        loop {
            ...
            let current_real_idx = filtered_indices
                .get(selected_filtered_pos)
                .copied()
                .unwrap_or(0);
            ...
            let (has_detail, has_comments) = {
                ...
                (
                    details.contains_key(&current_task.id),
                    comments.contains_key(&current_task.id),
                )
            };
            ...
            if (!has_detail || !has_comments) && !loading_tasks.contains(&current_task.id) {
                ...
                tokio::spawn(async move {
                    ...
                    let detailed = match api_clone.get_task_detail(&task_id).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "Failed to fetch task details for {}: {:?}",
                                task_id,
                                e
                            );
                            task_fallback
            ...
            let detailed_task = cached_task_details
                .lock()
                .unwrap()
                .get(&current_task.id)
                .cloned();
            let comments = cached_comments
                .lock()
                .unwrap()
                .get(&current_task.id)
                .cloned();
            ...
            let right_pane_widget = match (&detailed_task, &comments) {
                (Some(detailed_task), Some(comments)) => {
                    ...
                    let desc_text = detailed_task
                        .text_content
                        .as_deref()
                        .unwrap_or("No description");

                    let status_color =
                        crate::ui::styles::get_status_color(&detailed_task.status.status);
                    detail_lines.push(Line::from(vec![
                        Span::styled(
                            "Status:    ",
                            Style::default()
                                .add_modifier(Modifier::BOLD)
                                .fg(crate::ui::styles::COLOR_MUTED),
                        ),
                        Span::styled(
                            detailed_task.status.status.to_uppercase(),
                            Style::default()
                                .add_modifier(Modifier::BOLD)
                                .fg(status_color),
                        ),
                    ...
                    let task_url = format!(
                        ...
                        cfg.workspace_id, detailed_task.id
                    );
                    let mut link_spans = vec![Span::styled(
                        "Link:      ",
                        Style::default()
                            .add_modifier(Modifier::BOLD)
                            .fg(crate::ui::styles::COLOR_MUTED),
                    ...
                    let wrapped_desc =
                        crate::util::format::wrap_text_by_words(desc_text, bottom_pane_width);
                    ...
                    if comments.is_empty() {
                        ...
                        for c in comments {
                            ...
                            let wrapped_comment = crate::util::format::wrap_text_by_words(
                                &c.comment_text,
                                bottom_pane_width.saturating_sub(2),
                            );
            ...
            terminal.draw(|f| {
                ...
                if state == BrowseState::CommentEditor {
                    ...
                    let items: Vec<ListItem> = list_statuses
                        .iter()
                        .map(|s| {
                            ListItem::new(format!("  {}", s.status)).style(
                                Style::default()
                                    .fg(crate::ui::styles::COLOR_FG)
                                    .bg(crate::ui::styles::COLOR_BG),
                            )
                    ...
                    let items: Vec<ListItem> = space_tags
                        ...
                        .enumerate()
                        .map(|(i, tag)| {
                            let checked = if task_tag_selection.get(i).copied().unwrap_or(false) {
                                "[x]"
                            } else {
                                "[ ]"
                            };
                    ...
                    let items: Vec<ListItem> = space_tags
                        ...
                        .enumerate()
                        .map(|(i, tag)| {
                            let checked = if filter_tag_selection.get(i).copied().unwrap_or(false) {
                                "[x]"
                            } else {
                                "[ ]"
                            };
            ...
            let detail_tags_snapshot: std::collections::HashMap<String, Vec<String>> = {
                ...
                details
                    ...
                    .map(|(id, t)| {
                        (
                            id.clone(),
                            t.tags.iter().map(|tag| tag.name.clone()).collect(),
                        )
                    })
                    .collect()
            };
                        ...
                        match state {
                            BrowseState::List => match key.code {
                                ...
                                KeyCode::Char('T') => {
                                    ...
                                    filter_tag_selection = space_tags
                                        .iter()
                            ...
                            BrowseState::StatusPicker => match key.code {
                                ...
                                KeyCode::Enter => {
                                        ...
                                        .is_ok()
                                    {
                                        ...
                                        loading_tasks.remove(&current_task.id);
                                        if let Some(t) =
                                            tasks.iter_mut().find(|t| t.id == current_task.id)
                                        {
                            ...
                            BrowseState::TagPicker => match key.code {
                                ...
                                KeyCode::Enter => {
                                    ...
                                    for (i, tag) in space_tags.iter().enumerate() {
                                        let wanted =
                                            task_tag_selection.get(i).copied().unwrap_or(false);
                                        let had = current_task_tags.contains(&tag.name);
                                        if wanted && !had {
                                            if api
                                                .add_tag_to_task(&task_id, &tag.name)
                                                .await
                                                .is_err()
                                            {
                                                any_err = true;
                                            ...
                                            && had
                                            && api
                                                .remove_tag_from_task(&task_id, &tag.name)
                                                .await
                                                .is_err()
                                        {
                                            any_err = true;
                            ...
                            BrowseState::FilterEditor => match key.code {
                                KeyCode::Esc => {
                                    ...
                                    filter_query.clear();
                                    filtered_indices = compute_filtered_indices(
                                        &tasks,
                                        "",
                                        &active_filter_tags,
                                        &detail_tags_snapshot,
                                    );
                            ...
                            BrowseState::FilterTagPicker => match key.code {
                                ...
                                KeyCode::Enter => {
                                    active_filter_tags = space_tags
                                        .iter()
                                        .enumerate()
                                        .filter(|(i, _)| {
                                            filter_tag_selection.get(*i).copied().unwrap_or(false)
                                        })
    ...
    terminal.draw(|f| {
        ...
        let lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    sub_message,
                    Style::default().fg(crate::ui::styles::COLOR_WARN),
                ),
            ]),
            Line::from(""),
            Line::from(vec![Span::styled(
                "  Please wait while we sync with ClickUp...",
                Style::default()
                    .fg(crate::ui::styles::COLOR_MUTED)
